...
class Template(object):
    ...
    __structure_patterns_list = {'if': __if_condition_pattern, 'var': __variable_condition_symbols}
    ...
    def render(self, **kwargs):
        ...
        for key, pattern in self.__structure_patterns_list.items():
            temp_var = re.finditer(pattern, self.fragment)
            if temp_var:
                if key == 'var':
                    for i in temp_var:
                        temp_string = temp_string.replace(i.group(), self._variable_render(kwargs, i.group()))
                elif key == "if":
                    for i in temp_var:
                        temp_string = temp_string.replace(i.group(), self._condition_render(kwargs, i.group()))
        return temp_string
        ...
        for condition in conditions_list:
            ...
            for key, value in kwargs.items():
                if temp_condition[0].find(key) != -1:
                    temp_condition[0] = temp_condition[0].replace(key, str(value))
...
# text = 'Your result: {?count == 5:it\'s five, count < 5:less than five, else:more than five?}!'
# test = Template(text)
# print(test.render(count = 55))
